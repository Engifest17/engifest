...
camera.position.z = sun.sunFactorCamera*sun.size;
  ...
  var nearness;
  var sunFarPoint = 300;
  var cameraMovingFarPoint = 150;
  var cameraMovingNearPoint = 100;
  randomPoints.push(
      new THREE.Vector3(sun.planet.position.x, sun.planet.position.y, 2*sun.size),
      new THREE.Vector3(mercury.planet.position.x, mercury.planet.position.y+5, sunFarPoint),
...
for (var i = 0; i < 6000; i++) {
  ...
  star.mesh.position.z = minDistanceZ + Math.random() * (maxDistanceZ - minDistanceZ);
